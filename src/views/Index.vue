...
export default {
  ...
  methods: {
    ...
    onMapload(map) {
      ...
      var carlayer = new mars3d.layer.GraphicLayer()
      map.addLayer(carlayer)

      // initLayerManager(carlayer)
        //加一些演示数据
        for (let i = 0; i < 20; i++) {
          ...
          carlayer.addGraphic(graphic)
        }

        // 设置动态位置
        carlayer.eachGraphic((graphic) => {
          graphic.addDynamicPosition(this.randomPoint())
        })
        carlayer.eachGraphic((graphic) => {
          graphic.addDynamicPosition(this.randomPoint(), 20)
        })
      ...
      this.addSaomiao(graphicLayer)
      this.addweiqiang(graphicLayer)
    },


    randomPoint(){
        var jd = haoutil.math.random(120.9399 * 1000, 121.875712 * 1000) / 1000
        var wd = haoutil.math.random(31.462645 * 1000, 30.764256 * 1000) / 1000
        return this.Cesium.Cartesian3.fromDegrees(jd, wd, 30)
    },

    addSaomiao(graphicLayer){
     var _rotation = Math.random()
        var graphic = new this.mars3d.graphic.CircleEntity({
          position: Cesium.Cartesian3.fromDegrees(121.469501, 31.2301, 27.88),
          style: {
            radius: 50000.0,
            //扫描材质
    ...
    addweiqiang(graphicLayer){
      //立体围墙扩散效果,圆状
      var circleDiffuseWallGlow = new this.mars3d.graphic.DiffuseWall({
        position: new mars3d.LatLngPoint(121.469501, 31.2301, 44.3), //圆中心点
        style: {
          diffHeight: 100000, //高度
          radius: 100000, //半径
          color: '#ff0000',
          speed: 10, //速度
        },
